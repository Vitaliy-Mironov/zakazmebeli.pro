...
if ($b499 == "1") {
	$d600 = '<p class="direction-p">Если кухня не упирается в боковые стены, то проверять соответствие углов 90&deg; не требуется. Просто подтвердите установленные данные.</p>';
} else {
	$d600 = '<p class="direction-p">Проверьте угломером неизмеренные углы и впишите данные Ваших замеров.</p>
		<p class="direction-p">Вводить числа нужно в формате XX.X или XXX.X с точкой в качестве разделителя. Например: 90.0 или 89.7 или 101.2</p>';
}
...
<form class="detailing-form step">
	<fieldset class="detailing-form-fieldset">
		<div class="direction">
			...
			<?php
			if ($b499 == "1") {echo '<p class="direction-p">Если кухня не упирается в боковые стены, то проверять соответствие углов 90&deg; не требуется. Просто подтвердите установленные данные.</p>'."\n";}
			else {echo '<p class="direction-p">Проверьте угломером неизмеренные углы и впишите данные Ваших замеров.</p>
			<p class="direction-p">Вводить числа нужно в формате XX.X или XXX.X с точкой в качестве разделителя. Например: 90.0 или 89.7 или 101.2</p>'."\n";}
			?>
			<p class="direction-p"><a href="kak-pravilno-izmerit-ugol.php" class="direction-p-link icon-doc">Как правильно измерить угол?</a></p>
			<p class="direction-p"><a href="kak-izmerit-ugol-bez-uglomera.php" class="direction-p-link icon-doc">Как измерить угол без угломера?</a></p>
		...
		<div class="detailing-form-input-div">
			...
			Угол 1: <input name="UGLzaz000" type="text" class="textnumber underline" pattern="[0-9]{2,3}.[0-9]{1}" value="<?php echo $b61; ?>" required<?php echo $b61b; ?> />
			</label>
		</div>
		<div class="detailing-form-input-div">
			<label class="textlabel">
			Угол 2: <input name="UGLzdz000" type="text" class="textnumber underline" pattern="[0-9]{2,3}.[0-9]{1}" value="<?php echo $b62; ?>" required<?php echo $b62b; ?> />
			</label>
		</div>
		<div class="detailing-form-input-div<?php if ($c < 3) {echo ' hidden';} ?>">
			<label class="textlabel">
			Угол 3: <input name="UGLzgz000" type="text" class="textnumber underline" pattern="[0-9]{2,3}.[0-9]{1}" value="<?php echo $b63; ?>"<?php if ($c >= 3) {echo ' required';} ?><?php echo $b63b; ?> />
			</label>
		</div>
		<div class="detailing-form-input-div<?php if ($c < 4) {echo ' hidden';} ?>">
			<label class="textlabel">
			Угол 4: <input name="UGLzjz000" type="text" class="textnumber underline" pattern="[0-9]{2,3}.[0-9]{1}" value="<?php echo $b64; ?>"<?php if ($c >= 4) {echo ' required';} ?><?php echo $b64b; ?> />
			</label>
		</div>
		<div class="detailing-form-input-div<?php if ($c < 5) {echo ' hidden';} ?>">
			<label class="textlabel">
			Угол 5: <input name="UGLzmz000" type="text" class="textnumber underline" pattern="[0-9]{2,3}.[0-9]{1}" value="<?php echo $b65; ?>"<?php if ($c >= 5) {echo ' required';} ?><?php echo $b65b; ?> />
			</label>
		</div>
...
switch ($_SESSION["UGLzxz000"]) {
	case "ok":
		require 'kitchen_step_00700.php';
		break;
}
?>