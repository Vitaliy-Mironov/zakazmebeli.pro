...
<form class="detailing-form step">
	<fieldset class="detailing-form-fieldset">
		<div class="instruction">
			...
			<p class="instruction-p">Измерьте и запишите указанное на изображении расстояние (в миллиметрах).</p>
			<p class="instruction-p"><a href="kak-pravilno-izmerit-razmery-pod-kuhnyu.php" class="instruction-p-link icon-doc" target="_blank">Как правильно измерить размеры под кухню, и какие моменты учесть?</a></p>
		...
		<div class="detailing-form-input-div">
			...
			Размер 1, мм: <input onkeydown="if(event.keyCode==13){return false;}" name="UGLazd000" type="number" class="textnumber big underline" pattern="[0-9]" min="1" max="10000" value="<?php echo $b71; ?>" required />
			</label>
		</div>
		<div class="detailing-form-input-div<?php if ($c < 2) {echo ' hidden';} ?>">
			<label class="textlabel big">
			Размер 2, мм: <input onkeydown="if(event.keyCode==13){return false;}" name="UGLdzg000" type="number" class="textnumber big underline" pattern="[0-9]" min="1" max="10000" value="<?php echo $b72; ?>"<?php if ($c >= 2) {echo ' required';} ?> />
			</label>
		</div>
		<div class="detailing-form-input-div<?php if ($c < 3) {echo ' hidden';} ?>">
			<label class="textlabel big">
			Размер 3, мм: <input onkeydown="if(event.keyCode==13){return false;}" name="UGLgzj000" type="number" class="textnumber big underline" pattern="[0-9]" min="1" max="10000" value="<?php echo $b73; ?>"<?php if ($c >= 3) {echo ' required';} ?> />
			</label>
		</div>
		<div class="detailing-form-input-div<?php if ($c < 4) {echo ' hidden';} ?>">
			<label class="textlabel big">
			Размер 4, мм: <input onkeydown="if(event.keyCode==13){return false;}" name="UGLjzm000" type="number" class="textnumber big underline" pattern="[0-9]" min="1" max="10000" value="<?php echo $b74; ?>"<?php if ($c >= 4) {echo ' required';} ?> />
			</label>
		</div>
		<input onkeydown="if(event.keyCode==13){return false;}" name="UGLxzx000" type="text" class="hidden" value="ok" /> <!-- контролька -->
	</fieldset>
	<a id="UGLxzx000"></a>
