...
if (!empty($_REQUEST["ZFNbot001"])) {$b151 = $_REQUEST["ZFNbot001"];} elseif (!empty($_SESSION["ZFNbot001"])) {$b151 = $_SESSION["ZFNbot001"];} elseif ($_SESSION["MDNfas000"] == "imposed") {$b151 = "2.0" + $_SESSION["STLksb000"];} else {$b151 = "2.0";}
...
if (!empty($_REQUEST["ZFVbot000"])) {$b155 = $_REQUEST["ZFVbot000"];} elseif (!empty($_SESSION["ZFVbot000"])) {$b155 = $_SESSION["ZFVbot000"];} else {$b155 = "4.0";}
...
<form class="detailing-form step">
	<fieldset class="detailing-form-fieldset">
		...
		<legend class="headline"><?php echo $formcount.'. '; ?>Зазоры нижних модулей</legend>
		...
		<div class="detailing-form-input-div">
			...
			A. Верхний зазор, мм: <input onkeydown="if(event.keyCode==13){return false;}" name="ZFNbot001" type="text" class="textnumber underline" pattern="[0-9]{1,2}.[0-9]{1}" value="<?php if (preg_match("/\./", $b151)) {echo $b151;} else {echo $b151.".0";} ?>" required />
			</label>
		</div>
		<div class="detailing-form-input-div">
			<label class="textlabel big">
